...
//        $this->assign('page', $data->render());
    ...
    public function guanyu2()
    {
        $content = hook_one('portal_admin_article_index_view');
        $name=$this->request->param('name');
//        dump($name);
        $id=Db::table('auto_portal_category')->where('name',$name)->field('id')->find();
        if (!empty($content)) {
            return $content;
        }
...
//            dump($id);
    ...
    public function edit()
    {
        ...
        $ide=Db::table('auto_portal_category_post')->where('post_id',$id)->field('category_id')->find();
...
//dump($postCategoryIds);
    ...
    public function message(){
      $str=Db::table('auto_message')->paginate(30);
      $this->assign('str',$str);
      return $this->fetch();
    }
...
//        dump($files);
        ...
        $data['post_keywordsen']=$this->request->param('post_keywordsen');
        $data['post_description']=$this->request->param('post_description');
        $data['post_descriptionen']=$this->request->param('post_descriptionen');
        $data['post_source']=$this->request->param('post_source');
        $data['post_sourceen']=$this->request->param('post_sourceen');
        $data['post_excerpt']=$this->request->param('post_excerpt');
        $data['post_excerpten']=$this->request->param('post_excerpten');
        $data['post_content']=$this->request->param('post_content');
        $data['post_contenten']=$this->request->param('post_contenten');
        $data['post_source']=0;
        $data['delete_time']=0;
        $datae['post_title']=$this->request->param('title');
        $datae['post_titleen']=$this->request->param('post_titleen');
        $var=explode(",",$datae['post_title']);
        $varen=explode(",",$datae['post_titleen']);
...
//                echo $info->getFilename();
        ...
        if ($sto==true){
            ...
            $url = url('gf',['name'=>$conid['name'],'id'=>$id]);
...
//        die;
        ...
        $num = Db::table('auto_portal_post')->insertGetId($data);
        $me['post_id']=$num;
        $me['category_id']=$id;
        $me['list_order']=10000;
        $me['status']=1;
        Db::table('auto_portal_category_post')->insert($me);
        if ($num) {
