...
class AdminItemController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        $items         = $mallItemModel->where($where)->order('create_time DESC')->paginate(10);
    ...
    public function addPoster(){
        ...
        $datae['title']=$this->request->param('title');
        $datae['titlem']=$this->request->param('titlem');
        $varen=explode(",",$datae['titlem']);
        $var=explode(",",$datae['title']);
        ...
        $data['contentr']=$this->request->param('contentr');
        $data['contenth']=$this->request->param('contenth');
        $data['price']=$this->request->param('price');
        $data['original_price']=$this->request->param('original_price');
        $data['content']=$this->request->param('content');
        $data['contentm']=$this->request->param('contentm');
        $data['create_time']=time();
        $data['update_time']=time();
        foreach($files as  $key => $file) {
            $info = $file->move(ROOT_PATH . 'upload' . DS . 'mall');
            $data['thumbnail']='mall/'.$info->getSaveName();
            if (empty($datae['titlem'])){
                $var=explode(",",$datae['title']);
                $data['title']=$var[$key];
            }else{
                $varen=explode(",",$datae['titlem']);
                $var=explode(",",$datae['title']);
                $data['title']=$var[$key];
                $data['titlem']=$varen[$key];
            }
            $sto=Db::table('auto_mall_item')->insert($data);
        }
        /*$data['more']['photos'] = [];
    ...
    public function edit()
    {
        ...
        $da=Db::table('auto_mall_category')->where('id',$item['category_id'])->find();
//        dump($da);
...
//        return ;
    ...
    {
        $param           = $this->request->param();
        $portalPostModel = new MallItemModel();
    ...
    public function delete()
    {
        ...
        if (isset($data['ids']) && !empty($data["sell"])) {
            $ids = $this->request->param('ids/a');
            $mallItemModel->where('id', 'in', $ids)->update(['delete_time' => time()]);
            $this->success("更新成功！");
        }

        if (isset($data['ids']) && empty($data["sell"])) {
            $ids = $this->request->param('ids/a');
            $mallItemModel->where('id', 'in', $ids)->delete();
            $this->success("成功！");
        }
    }
