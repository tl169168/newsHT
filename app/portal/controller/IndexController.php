...
class IndexController extends HomeBaseController
{
    function _initialize()
    {
        ...
        $en=url('en/index/indexs',['id'=>'en']);

        $index=url('portal/index/index',[]);

        $about=url('portal/index/about',['id'=>'cn']);
        $product=url('portal/index/product',['id'=>'cn']);
        $equipment=url('portal/index/equipment',['id'=>'cn']);
        $messages=url('portal/index/messages',['id'=>'cn']);
        $jdcgl=url('portal/index/jdcg',[]);
        $zhaopin=url('portal/index/zhaopin',['id'=>'cn']);
        $zxly=url('portal/index/zxly',[]);

        $news=url('portal/index/news',['id'=>'cn']);
        $faq=url('portal/index/faq',['id'=>'cn']);
        $contact=url('portal/index/contact',['eid'=>'90']);
        $this->assign('en', $en);
...
//公司新闻
        ...
        $gsxw4['url'] =url('portal/index/newsmore',['id'=>$gsxw4['post_id'],'eid'=>$gsxw4['category_id']]);
//        dump($gsxw4); return ;
...
//        return $eid;
        ...
        for($i=0;$i<count($fenlei);$i++){
            $fenlei[$i]['url']=url('portal/index/jdcg',['id'=>$fenlei[$i]['id'],'eid'=>$eid]);
        }
        $this->assign('fenlei', $fenlei);
        $zhapin=url('portal/index/job',['id'=>'cn']);
        $rencai=url('portal/index/job',['id'=>'ens']);
...
//         dump($news);exit;
        ...
        for ($c=0;$c<count($news['data']);$c++){
//           dump($news['data'][$c]['title']);
            $news['data'][$c]['url'] =url('portal/index/product',['id'=>$news['data'][$c]['id']]);
        }
//
        }
//        dump($news);
...
//        dump($news); return ;
        for ($c=0;$c<count($news['data']);$c++){
//           dump($news['data'][$c]['title']);
            $news['data'][$c]['url'] =url('portal/index/newsmore',['id'=>$news['data'][$c]['post_id']]);
        }
...
//        dump($gs);
        $gsxwt=Db::table('auto_portal_category a,auto_portal_category_post b,auto_portal_post c')  ->where('a.id=b.category_id')
            ...
            ->find();
        if (!empty($gsxwt)){
            $gsxwt['url']=url('portal/index/newsmore',['id'=>$gsxwt['post_id']]);
        ...
        $gsxwn=Db::table('auto_portal_category a,auto_portal_category_post b,auto_portal_post c')  ->where('a.id=b.category_id')
            ...
            ->find();
        if (!empty($gsxwn)){
            $gsxwn['url']=url('portal/index/newsmore',['id'=>$gsxwn['post_id']]);
    ...
    public function contact()
    {
        ...
        $contact1=url('portal/index/contact',['id'=>254]);
        $feedback=url('portal/index/feedback',['id'=>'cn']);
...
//            dump($gsxw);
    ...
    public function zhaopin()
    {
        ...
        for($i=0;$i<count($fenlei);$i++){
            $fenlei[$i]['url']=url('portal/index/zhaopin',['id'=>$fenlei[$i]['id'],'eid'=>$eid]);
        }
        $this->assign('fenlei', $fenlei);
        $this->assign('kewords',$kewords);
        $this->assign('kewordse',$kewordse);
